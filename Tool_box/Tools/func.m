...
%
%     Authors: Katja Lorbacher & Dietmar Dommenget (ddommenget@ifm-geomar.de)
...
if (length(imld)==0); imld=length(z); end
imld=max([imld,3]);

% ################     FUNCTION  mld_0level   #######################
function [imld, ipo] = mld_0level(z,t,imld0,gt,res,sig30)
...
% curvature
ct(1)=0.; for i=2:length(gt); ct(i)=(gt(i)-gt(i-1))./(z(i-1)-z(i)); end

% find local extreme in ct
i=1; imld=0;
while(imld==0 & i<imld0-1); i=i+1;
...
if (imld==0) ; imld=min(find(abs(gt(1:imld0))>0.7*max(abs(gt(1:imld0))))); end
...
if ( imld~=1 & imld<ldim )
    ...
    if ( resi < 6 )
        ...
        if ( imld+3 <= ldim )
            ipo=1;
            if (gt(imld+1)/gt(imld)>=1.0); ipo=2; end              % NO, not convex
            if (ipo==2 & abs(gt(imld+2)/gt(imld+1)-0.5)<0.5 ...
             & gt(imld-1)/gt(imld)<0.1 ); imld=imld+1; ipo=1;end   % but convex for imld+1

            if (abs(gt(imld+1)/gt(imld)-0.875)<0.125  ...
               & gt(imld-1)/gt(imld)>0.1);  ipo=2;end              % doubtful gradients
            if  (gt(imld+1)/gt(imld)<=0.0); ipo=2;end              % change of sign
        end
    end
    if (resi <6); sgt=sgth; end
    if (resi>=6); sgt=sgtl; end
    if (ipo==2 & ( abs(gt(imld-1)/gt(imld)-.05)<.05 | abs(gt(imld-1))<sgt ) ); imld=imld+1;end
end
...
if (ipo==1)
    ...
    if (s1-s2==0.); return;end
    s3=sum(x(1:2).^2.);      s4=sum(x(1:2)).*sum(x(1:2))./2;
    if (s3-s4==0.); return;end
    b=(s1-s2)./(s3-s4); a=sum(dy(1:2))./2-b.*sum(x(1:2))./2; B=-b;
    if (a<=0.); return;end
    A=exp(a)./B;
    C=1./(2+1).*(sum(t(imld:imld+2))-sum(A*exp(-abs(z(imld:imld+2)).*B)) );
    x1(1:3)=t(imld:imld+2); x2(1:3)=C+A.*exp(-B.*abs(z(imld:imld+2)));
    corrcoef(x1,x2); r=ans(1,2);
...
if (ipo==2);
   ...
   % linear interpolation would overshoot: find a dT to go down from imld-1
       dt=max( [0.01  max(abs(  diff(t(1:imld-1))' )) ] );
...
if (mld > z(max(1,imld-1)));    mld=z(max(1,imld-1));    end
if (mld < z(min(ldim,imld+1))); mld=z(min(ldim,imld+1)); end
...
if (imld > 1 & length(i2) > 0 )
    v1=std(t(1:imld)); v2=std(t(1:imld+i2));
    if (v2 ~= 0.0); qe=1-v1/v2; end;
end