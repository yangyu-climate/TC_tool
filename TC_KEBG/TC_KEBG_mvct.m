...
for T = T_beg:T_frq:T_end
    ...
    if ~isempty(filename)
        ...
        if IF_Zfix
            for i=1:size(x,1)
                for j=1:size(y,2)
                    ...
                    tkS(:,i,j)         = interp1(squeeze(z(:,i,j)),squeeze(tk(:,i,j)),z_hight);
                    thetaS(:,i,j)      = interp1(squeeze(z(:,i,j)),squeeze(theta(:,i,j)),z_hight);
                    thetaES(:,i,j)     = interp1(squeeze(z(:,i,j)),squeeze(thetaE(:,i,j)),z_hight);
                    H_DIABATICS(:,i,j) = interp1(squeeze(z(:,i,j)),squeeze(H_DIABATIC(:,i,j)),z_hight);
                end
            end
            z          = zS;
            P          = PS;
            ...
            clear zS PS uS vS wS rhoS tkS thetaS thetaES H_DIABATICS
        end
        ...
        clear u v w rho tk theta thetaE H_DIABATIC H_MICRO H_RAD H_PBL H_CU
